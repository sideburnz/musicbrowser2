        ...
        public IWebServiceDTO Fetch(IWebServiceDTO dto)
        {
            ...
            if (_provider.ResponseStatus == "200")
            {
                ...
                else
                {
                    ...
                    LoggerEngineFactory.Debug("AlbumInfoService", String.Format("Last.fm album look up for \"{0}\" by \"{1}\" but matched \"{2}\" instead", localDTO.Album, localDTO.Artist, lfmAlbumName));
                }
            }
            else
            {
